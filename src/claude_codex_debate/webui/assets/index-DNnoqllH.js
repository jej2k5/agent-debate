...
`).filter(r=>r.trim()!=="");if(a.length!==0)if(a.length===1&&/^#{1,6}\s+/.test(a[0]))e+=`<p><strong>${w(a[0].replace(/^#{1,6}\s+/,""))}</strong></p>`;else if(a.every(r=>/^\s*[-*]\s+/.test(r))){const r=a.map(c=>`<li>${w(c.replace(/^\s*[-*]\s+/,""))}</li>`).join("");e+=`<ul>${r}</ul>`}else if(a.every(r=>/^\s*\d+[.)]\s+/.test(r))){const r=Number(((t=a[0].match(/^\s*(\d+)/))==null?void 0:t[1])??"1"),c=a.map(o=>`<li>${w(o.replace(/^\s*\d+[.)]\s+/,""))}</li>`).join("");e+=`<ol${r!==1?` start="${r}"`:""}>${c}</ol>`}else e+=`<p>${a.map(w).join("<br>")}</p>`}return e}const Y=typeof navigator<"u"&&/Mac|iPhone|iPad/.test(navigator.platform),Q=new Set(["the","a","an","is","are","of","for","to","in","and","or"]);function ee(s){const e=s.toLowerCase().replace(/[^a-z0-9]+/g,"-").replace(/^-+|-+$/g,"").split("-").filter(Boolean).slice(0,5);for(;e.length>2&&Q.has(e[e.length-1]);)e.pop();return e.join("-").slice(0,40)}function te(s){const e=s.trim(),t=e.match(/^.*?[.!?](?=\s|$)/);return(t?t[0]:e).trim()}function q(s){return s.trim().replace(/[\s;,]+$/,"")}function A(s){const e=s.replace(/\s+/g," ").trim(),t=e.match(/\b(?:common ground|consensus|shared conclusion|conclusion)\b\s*[:—-]\s*(.+)/i);if(t){const r=t[1].split(/\s*[;.]\s+(?=remaining (?:differences|disagreement)\b|key assumptions\b|remaining\b|differences\b)/i)[0];return q(r)}const n=e.match(/\(1\)\s*(.+?)(?=\s*\(2\)|$)/);if(n)return q(n[1].replace(/^[A-Z][A-Z0-9 /,-]{2,}?:\s*/,""));const a=e.replace(/^(?:agreement|agreed|consensus|both agents? agree|we agree)[^.:]*[.:]\s*/i,"");return q(te(a||e))}function p(s){return s==="claude"?"Claude":s==="codex"?"Codex":s==="moderator"?"Moderator":s}const g=new Map;function I(s){const e=window.setTimeout(()=>g.delete(s),15e3),t=g.get(s);t&&window.clearTimeout(t),g.set(s,e)}function x(s){const e=g.get(s);e&&window.clearTimeout(e),g.delete(s)}const se={claude:["opus","sonnet","haiku"],codex:["gpt-5.2-codex","gpt-5-codex","gpt-5.1-codex-mini"]};function C(s,e="Launch terminal"){if(g.has(s))return`<button class="btn btn--ghost btn--sm" type="button" data-launch="${s}" disabled>Opened &mdash; registering&hellip;</button>`;const t=`models-${s}`,n=se[s].map(a=>`<option value="${a}"></option>`).join("");return`
    ...
    <section class="system-strip" aria-label="System status">
    ...
    </section>`}function L(s){const e=t=>{var n;return((n=t.closest("[aria-label], .rail, .empty"))==null?void 0:n.querySelector("[data-system-error]"))??s.querySelector("[data-system-error]")};s.querySelectorAll("[data-launch]").forEach(t=>{t.addEventListener("click",async()=>{var o;const n=t.dataset.launch,a=(o=t.closest(".launch-control"))==null?void 0:o.querySelector("[data-model]"),r=(a==null?void 0:a.value.trim())||void 0,c=t.textContent??"Launch terminal";t.disabled=!0,t.textContent="Opening…",I(n);try{await Z(n,r),t.textContent="Opened — registering…"}catch(i){x(n),t.disabled=!1,t.textContent=c;const u=e(t);u&&(u.textContent=i instanceof Error?i.message:"Launch failed.")}})}),s.querySelectorAll("[data-start-bridge]").forEach(t=>{t.addEventListener("click",async()=>{t.disabled=!0,t.textContent="Starting…",I("bridge");try{await J()}catch(n){x("bridge"),t.disabled=!1,t.textContent="Start bridge";const a=e(t);a&&(a.textContent=n instanceof Error?n.message:"Couldn't start the bridge.")}})})}function ae(s,e){return`${s==="claude"?"Use the debate-peer skill.":"Use $debate-peer."} Join debate ${e}. Choose your own position and continue until you agree or explicitly agree to disagree.`}class re{constructor(e,t){l(this,"root");l(this,"debateId");l(this,"data",null);l(this,"lastMaxId",0);l(this,"firstRender",!0);l(this,"optimistic",null);l(this,"composerError","");l(this,"notFound",!1);l(this,"renderedDelivered",new Map);l(this,"optimisticEl",null);l(this,"announcedMaxId",null);l(this,"announcedResolutionId",null);l(this,"announcedStatus",null);this.root=e,this.debateId=t,this.root.innerHTML=`
      <div class="room">
      ...
      </div>`}focusHeading(){var e;(e=this.root.querySelector("[data-topic]"))==null||e.focus({preventScroll:!0})}async refresh(){try{this.data=await U(this.debateId)}catch(e){e instanceof S&&e.status===404&&(this.notFound=!0,this.renderNotFound());return}this.notFound||(this.optimistic&&this.data.messages.some(e=>e.sender==="moderator"&&e.content===this.optimistic.content)&&(this.optimistic=null),this.announceChanges(),this.render())}announceChanges(){var r;if(!this.data)return;const{debate:e,messages:t}=this.data,n=t.reduce((c,o)=>Math.max(c,o.id),0),a=((r=e.pending_resolution)==null?void 0:r.id)??null;if(this.announcedMaxId===null){this.announcedMaxId=n,this.announcedResolutionId=a,this.announcedStatus=e.status;return}for(const c of t)c.id>this.announcedMaxId&&c.sender!=="moderator"&&$(`${p(c.sender)} replied`);if(this.announcedMaxId=n,a!==this.announcedResolutionId&&e.pending_resolution){const c=e.pending_resolution,o=c.proposer==="claude"?"Codex":"Claude";$(`Resolution proposed by ${p(c.proposer)}, awaiting ${o}`)}this.announcedResolutionId=a,e.status!==this.announcedStatus&&e.status==="completed"&&$(e.resolution_type==="agreement"?"Agreement reached":"Agreed to disagree"),this.announcedStatus=e.status}renderNotFound(){this.root.innerHTML=`
      <div class="overview">
        <section class="empty">
      ...
      </div>`}nearBottom(){return window.innerHeight+window.scrollY>=document.body.scrollHeight-200}render(){var o;if(!this.data)return;const{debate:e}=this.data,t=this.nearBottom(),n=this.root.querySelector("[data-topic]");n.textContent!==e.topic&&(n.textContent=e.topic);const a=`${e.topic} — Agent Debate`;document.title!==a&&(document.title=a),this.root.querySelector("[data-sub]").innerHTML=[`<span>${d(e.id)}</span>`,`<span>opened ${v(e.created_at)}</span>`,e.status==="completed"?`<span class="pill pill--done">${e.resolution_type==="agreement"?"Agreement":"Agreed to disagree"}</span>`:""].filter(Boolean).join(""),this.renderTranscript(),this.renderRail(),L(this.root.querySelector("[data-rail]")),this.renderComposer(),M(this.root);const r=(((o=this.data)==null?void 0:o.messages)??[]).reduce((i,u)=>Math.max(i,u.id),0),c=r>this.lastMaxId;this.firstRender?window.scrollTo({top:document.body.scrollHeight}):c&&t?window.scrollTo({top:document.body.scrollHeight,behavior:"smooth"}):c&&this.showNewBelow(),this.lastMaxId=Math.max(this.lastMaxId,r),this.firstRender=!1}messageHtml(e){var h;const t=e.id>this.lastMaxId&&!this.firstRender,n=e.sender,a=n==="moderator"?"msg--moderator":n==="claude"?"msg--claude":"msg--codex",r=e.optimistic?" is-pending":"",c=t?" is-new":"",o=!e.delivered_at&&!e.optimistic&&((h=this.data)==null?void 0:h.debate.status)==="running"?`<span class="msg-delivery" data-delivery>awaiting delivery to ${p(e.recipient)}</span>`:"",i=e.optimistic?'<span class="msg-delivery">sending&hellip;</span>':"",u=e.optimistic?"":v(e.created_at,"msg-time");return`
      <article class="msg ${a}${r}${c}" data-msg-id="${e.optimistic?"optimistic":e.id}">
        <div class="msg-head">
      ...
      </article>`}renderTranscript(){var c,o;if(!this.data)return;const{debate:e}=this.data,t=this.root.querySelector("[data-messages]"),n=Object.keys(e.participants);for(const i of this.data.messages){const u=!!i.delivered_at;this.renderedDelivered.has(i.id)?u&&this.renderedDelivered.get(i.id)===!1&&((c=t.querySelector(`[data-msg-id="${i.id}"] [data-delivery]`))==null||c.remove(),this.renderedDelivered.set(i.id,u)):(t.insertAdjacentHTML("beforeend",this.messageHtml(i)),this.renderedDelivered.set(i.id,u))}this.optimistic&&!this.optimisticEl?(t.insertAdjacentHTML("beforeend",this.messageHtml(this.optimistic)),this.optimisticEl=t.querySelector('[data-msg-id="optimistic"]')):!this.optimistic&&this.optimisticEl&&(this.optimisticEl.remove(),this.optimisticEl=null),e.status!=="running"&&t.querySelectorAll("[data-delivery]").forEach(i=>i.remove());const a=[];if(e.status==="running"&&n.length<2){const i=["claude","codex"],u=b=>{var y;return!!((y=this.data.agents[b])!=null&&y.registered)},h=i.some(u),j=e.current_turn,D=j==="claude"?"codex":"claude",H=i.filter(b=>!u(b));a.push(`
        <div class="awaiting">
          <p><strong>This debate hasn&#8217;t started.</strong></p>
          <p style="margin-top: var(--s2)">Send the opening prompt to both terminals &#8212; ${p(j)} opens, ${p(D)} joins and waits.</p>
        ...
        <section class="rail-section">
          ...
          ${e.resolution_summary?`<p class="rail-outcome-summary" title="${d(e.resolution_summary)}">${d(A(e.resolution_summary))}</p>`:""}
        </section>`);const c=["claude","codex"].map(o=>{const i=e.participants[o],u=n[o],h=u!=null&&u.running?`<span class="v">${E(n,o)}</span>`:`<span class="v absent">${E(n,o)}</span>
    ...
    <a class="ledger-row" href="#/d/${encodeURIComponent(s.id)}" title="${d(s.topic)}">
      ...
      ${a}
      <span class="ledger-state">${le(s)}</span>
    ...
    </section>`}class pe{constructor(e){l(this,"root");l(this,"data",null);l(this,"formOpen",!1);l(this,"submitting",!1);this.root=e,this.root.innerHTML=`
      <div class="overview">
